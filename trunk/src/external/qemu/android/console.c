...
static int
do_sim_envelope( ControlClient  client, char*  args )
{
    char temp[1024];

    if (!args) {
        control_write( client, "KO: missing argument, try 'sim envelope <apdu>'\r\n" );
        return -1;
    }
    sprintf( temp, "AT+CRSM=194,0,0,0,%d,%s", strlen(args) >> 1, args );
...
{
    { "envelope", "send APDU to SIM",
    "'sim envelope' send APDU to SIM\r\n", NULL,
    do_sim_envelope, NULL },
