...
/**
 * SEEK service base class for channel resources.
 */
abstract class Channel implements IChannel, IBinder.DeathRecipient  {
	...
	Channel(int channelNumber, ISeekServiceCallback callback) {
		this.channelNumber = channelNumber;
		this.binder = callback.asBinder();
		try {
			binder.linkToDeath(this, 0);
		} catch (RemoteException e) {
			Log.e(SeekService.SEEK_SERVICE_TAG, "Failed to register client callback");
		}
	}
	...
	public void binderDied() {
		...
		try {
			Log.v(SeekService.SEEK_SERVICE_TAG, Thread.currentThread().getName() + " Client " + binder.toString() + " died");
			close();
