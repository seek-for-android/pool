                                   ...
                                   int                        num_sectors)
{
    ...
    while (num_sectors > 0) {
        if (libmsc_read(sector_number, s->buf)) {
                                    ...
                                    int                        num_sectors)
{
    ...
    while (num_sectors > 0) {
        ...
        if (libmsc_write(sector_number, s->buf)) {
...
void parse_msc_options(const char *options)
{
    if((options == NULL) || (strlen(options) == 0)) {
        derror("goldfish_mmc.c: -msc must be followed by a filename\n");
        exit(1);
    }

    if (!libmsc_init(options))
        printf("goldfish_mmc.c: Mobile Security Card found\n");
}
