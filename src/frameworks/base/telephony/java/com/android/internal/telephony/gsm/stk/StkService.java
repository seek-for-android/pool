...
public class StkService extends Handler implements AppInterface {
    ...
    private void handleProactiveCommand(CommandParams cmdParams) {
        ...
        case OPEN_CHANNEL:
            if (!cmdMsg.geTextMessage().responseNeeded) {
                ...
                ResponseData resp = openChannel(cmdMsg);
                if (resp == null)
                    resCode = ResultCode.CMD_DATA_NOT_UNDERSTOOD;
                sendTerminalResponse(cmdParams.cmdDet, resCode, false,
                        0, resp);
                return;
            }
            break;
        case CLOSE_CHANNEL:
            mCurrntCmd = cmdMsg;
            Intent intent = new Intent(AppInterface.SCWS_CMD_ACTION);
            intent.putExtra("SCWS CMD", cmdMsg);
        ...
        case SEND_DATA:
            mCurrntCmd = cmdMsg;
            intent = new Intent(AppInterface.SCWS_CMD_ACTION);
            intent.putExtra("SCWS CMD", cmdMsg);
            mContext.sendBroadcast(intent);
            return;
        default:
    ...
    private void handleCmdResponse(StkResponseMessage resMsg) {
        ...
        case PRFRMD_TONE_NOT_PLAYED:
            ...
            case SET_UP_CALL:
                ...
                return;
            case OPEN_CHANNEL:
                resp = openChannel(mCurrntCmd);
                if (resp == null)
                    resMsg.resCode = ResultCode.CMD_DATA_NOT_UNDERSTOOD;
                break;
            case RECEIVE_DATA:
                if (resMsg.channelData != null) {
                    resp = new ReceiveDataResponseData(resMsg.channelData,
                            resMsg.channelDataLength);
                } else {
                    resMsg.resCode = ResultCode.CMD_DATA_NOT_UNDERSTOOD;
                }
                break;
            case SEND_DATA:
                if (resMsg.channelData == null) {
                    resp = new SendDataResponseData(resMsg.channelDataLength);
                } else {
                    resMsg.resCode = ResultCode.CMD_DATA_NOT_UNDERSTOOD;
                }
                break;
    ...
    private ResponseData openChannel(StkCmdMessage cmdMsg) {
        StkCmdMessage.ChannelSettings params = cmdMsg.getChannelSettings();
 
        if (params.protocol ==
                InterfaceTransportLevel.TransportProtocol.TCP_SERVER)
            params.channel = 1;
        else
        if (params.protocol ==
                InterfaceTransportLevel.TransportProtocol.TCP_CLIENT_REMOTE)
            params.channel = 2;
        else
        if (params.protocol ==
                InterfaceTransportLevel.TransportProtocol.TCP_CLIENT_LOCAL)
            params.channel = 2;
        else
            return null;
        ...
        int channelStatus = (params.channel << 8); // TCP in closed state
        int bearerType = 0;
        return new OpenChannelResponseData(params.bufSize, channelStatus,
                bearerType);
    }
}
