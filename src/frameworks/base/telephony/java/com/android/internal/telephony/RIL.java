...
public final class RIL extends BaseCommands implements CommandsInterface {
    ...
    public void
    iccAPDU(int cla, int command, int channel, int p1, int p2, int p3,
            String data, Message result) {
        RILRequest rr;
        if (channel == 0) {
            rr = RILRequest.obtain(RIL_REQUEST_SIM_APDU, result);
        } else {
        ...
        if (RILJ_LOGD) riljLog(rr.serialString() + "> iccAPDU: " + requestToString(rr.mRequest)
 ...
 | sed -re 's/\{([^,]+),[^,]+,([^}]+).+/case \1: ret = \2(p); break;/'
            ...
            case RIL_REQUEST_SIM_APDU: ret =  responseICC_IO(p); break;
