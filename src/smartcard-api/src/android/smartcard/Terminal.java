...
public abstract class Terminal implements ITerminal {
		...
		try {
			...
			for (int i = firstChannelNumber; i < lastChannelNumber; i++) {
				try {
					...
					Log.v(TERMINAL_SERVICE_TAG, Thread.currentThread().getName() + " channel " + i + " closed");
				} catch (Exception exp) {
				}
			}
	 ...
	 * 
	 * @param aid
	 *            the aid which should be selected.
	 */
	 ...
	 * @param commandName
	 *            the name of the smart card command for logging purposes. May
	 *            be <code>null</code>.
	 * @return the response received.
	 * @throws CardException
	 *             if the transmit operation or the minimum response length
	 *             check or the status word check failed.
	 */
	public byte[] transmit(byte[] cmd, int minRspLength, int swExpected, int swMask, String commandName) throws CardException {
		byte[] rsp = null;
		try {
