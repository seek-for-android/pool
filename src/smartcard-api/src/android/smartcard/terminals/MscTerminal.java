...
final class MscTerminal extends Terminal {
	...
	protected void internalConnect() throws CardException {
		...
		try {
			byte[] response = Transmit(new byte[] { 0x20, 0x12, 0x01, 0x01, 0x00 });
	...
	public boolean isCardPresent() throws CardException {
		...
		try {
			byte[] response = Transmit(new byte[] { 0x20, 0x13, 0x00, (byte) 0x80, 0x00 });
			if (response == null)
				return false;
	...
	protected boolean isCardPresentInternal(String storageName) throws CardException {
		//Log.v(SmartcardService.SMARTCARD_SERVICE_TAG, "isCardPresentInternal - MSC path: " + storageName);
		if (storageName.equals(""))
		{
			return false;
		}
		...
		if (!isConnected) {
			try {
				//Log.v(SmartcardService.SMARTCARD_SERVICE_TAG, "isCardPresent-call to internalConnect");						
				internalConnect();
			} catch (Exception e) {
				//Log.v(SmartcardService.SMARTCARD_SERVICE_TAG, "isCardPresentInternal - connect failed - returns false");
				return false;
			}
		}
		...
		try {
			...
			if (response == null)
			{
				//Log.v(SmartcardService.SMARTCARD_SERVICE_TAG, "isCardPresentInternal - response is NULL");
				return false;
			}	
			
			if (response.length == 0x05 && response[response.length - 2] == (byte) 0x90 && response[response.length - 1] == 0x00)
			{
				//Log.v(SmartcardService.SMARTCARD_SERVICE_TAG, "isCardPresentInternal - returns true");			
				return true;
			}
			...
			while ((rs = rd.readLine()) != null) {
				...
				for (String dir : dirs) {
					...
					if (path.isDirectory() && path.canWrite()) {
						storageName = path.getAbsolutePath() + "/msc.sig";
						try {
							internalConnect();
							if (isCardPresentInternal(storageName)) {
								Log.v(SmartcardService.SMARTCARD_SERVICE_TAG, "MSC path: " + storageName);
								return;
							} else {
								internalDisconnect();
								storageName = "";
	 ...
	 */
	private static native byte[] Transmit(byte[] command)
			throws Exception;
}
